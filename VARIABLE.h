...
public:
	...
	void toBin() const
	{
		...
		std::cout << "符号部s：" << sign << " (" << (sign ? "マイナス" : "プラス") << "を表す)\n";
		std::cout << "指数部e：" << exponent << "+127=" << exponent + 127;
		std::cout << " (最左の1の次まで'.'を" << exponent << "桁移動する。127を足すと指数部。)\n";
		std::cout << "仮数部m：移動した小数点より右側の23ビット\n";
